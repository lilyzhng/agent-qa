...
0. **Go/no-go: function calling against the internal model.** hello_world proved chat, not tools. Add one `@function_tool` to it, set `model=` to the internal model Aqua will use, confirm the model calls the tool and consumes the result. If this fails (serving with tool-calling disabled / model not trained for it), STOP and escalate — the fallback changes the whole port.
