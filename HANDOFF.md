...
- **Findings 2026-08-06 (work-side agent + Lily's correction):** data-bae = MCP server `great_gadsbq` in the separate `~/lat/data-platform` repo (binary at `scripts/.wrapped/great_gadsbq`); skills scalex-bae / umf-bae / streamlat-bae already use it. BQ auth = `gcloud auth application-default login`.
- **IMPORTANT — there is no dedicated traffic-sign table, and that's expected:** traffic signs are ONE OBJECT CLASS inside the general object-label tables. Don't search for a sign table; write a query function that filters the class column (`WHERE class = <traffic-sign enum>`). Autolabeling artifacts live in LakeFS with metadata in `scalex_datawatch_source` tables — relevant for pixels, not a reason to bypass BQ for labels.
- Phase 1 stays plain `google-cloud-bigquery` (+ LakeFS client if frames need it) as `@function_tool`s, NOT MCP (wheel constraint). Phase 2 = wire `great_gadsbq` as MCP server post-Dagster-upgrade.
- Survey before tool code — (1) which table holds object labels + exact class taxonomy value(s) for traffic signs, incl. sign-subtype/value fields Aqua needs; (2) the `list_items` query = that table filtered by class + dataset tag; (3) where referenced frames live and whether crops are pre-materialized (→ `view_sample`); (4) auth for the pixel store (BQ ADC doesn't cover LakeFS).
